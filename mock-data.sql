...
select department from departments where 38 < (select count(*) from employees e where e.department = departments.department) 
...
select first_name,hire_date,salary,sum(salary) Over(order by hire_date range between
													UNBOUNDED preceding and current row) as
running_total_sal
from employees

select first_name,hire_date,salary,sum(salary) Over(order by hire_date ROWS between
													1 preceding and current row) as
running_total_sal
from employees

RANK
-
select * from (select first_name,email, department,salary,RANK() OVER(PARTITION BY department order by salary desc)
from employees)a
where rank = 8


select first_name,email, department,salary,RANK() OVER(PARTITION BY department order by salary desc)
from employees
...
nth_value(salary, 5) OVER(PARTITION BY department order by first_name asc) first_val
from employees
...
select first_name,email, department,salary,
lead(salary) OVER() previous_sal
from employees

select first_name,email, department,salary,
LAG(salary) OVER() previous_sal
from employees
...
select first_name,email, department,salary,
LAG(salary) OVER(ORDER BY SALARY DESC) NEXT_HIH_SAL
from employees

select first_name,email, department,salary,
LEAD(salary) OVER(ORDER BY SALARY DESC) Closest_low_sal
from employees

select first_name,email, department,salary,
LAG(salary) OVER(PARTITION BY department ORDER BY SALARY DESC) NEXT_HIH_SAL
from employees


Rollup, grouping, cube

select * from sales order by continent, country
